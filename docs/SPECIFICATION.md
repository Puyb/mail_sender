...
- **Vitesse d'envoi** : paramètre de configuration globale serveur (pas par campagne/utilisateur).
...
| POST `/campaigns` | oui | `{draftUid, recipients}` → le backend **re-télécharge lui-même le MIME brut du brouillon via IMAP** (jamais reconstruit côté front, pour ne jamais faire transiter les pièces jointes par le navigateur), crée la campagne (status `pending`) + lance l'envoi en tâche de fond, retourne `{campaignId}` immédiatement |
| GET `/campaigns` | oui | liste filtrée par `sender_email = session.email`, avec compteurs d'ouverture/clic agrégés |
| GET `/campaigns/:id` | oui | détail + destinataires (avec `opened_at`, `clicked_at`) + breakdown par lien (`campaign_links` + comptage `tracking_events`) ; 403 si `sender_email` ne correspond pas à la session |
| GET `/t/o/:trackingId.png` | **non (public)** | pixel de tracking : enregistre un `tracking_event` de type `open` (met à jour `opened_at` si absent + incrémente `open_count`), répond toujours par un GIF/PNG 1×1 transparent, même si `trackingId` inconnu |
| GET `/t/c/:trackingId/:linkId` | **non (public)** | tracking de clic : enregistre un `tracking_event` de type `click` (met à jour `clicked_at` si absent + incrémente `click_count`), puis redirige (302) vers `campaign_links.original_url` |

Socket.IO : client → serveur `campaign:subscribe {campaignId}` (rejoint la room après vérif d'appartenance) ; serveur → client `campaign:progress {campaignId, sentCount, failedCount, totalRecipients, currentRecipientEmail}`, `campaign:completed {...,durationMs}`, `campaign:error {campaignId, message}`.
...
9. Redémarrer le process backend, se reconnecter, vérifier que la campagne terminée reste dans `/campaigns` avec ses stats d'envoi ET de tracking exactes (lues depuis SQLite).
