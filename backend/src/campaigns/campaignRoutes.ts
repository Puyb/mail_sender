...
export function createCampaignRouter(config: AppConfig, io: Server): Router {
  ...
  router.post('/', requireAuth, async (req, res) => {
    ...
    try {
      const campaign = await createAndStartCampaign(config, io, email, creds, parsed.data.draftUid, parsed.data.recipients);
  ...
  router.get('/:id', requireAuth, (req, res) => {
    ...
    res.json({ campaign: { ...campaign, raw_mime: undefined }, recipients, tracking, links });
  });
